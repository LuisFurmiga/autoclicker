...
def _send_combo(hwnd, tokens: list[str]):
    # Suporta "ctrl+c", "ctrl+shift+tab", etc.
    ...
    for m in mods:
        _post_key(hwnd, _MOD_VK[m], True, is_sys=is_sys)
        time.sleep(0.005)
    ...
    for m in reversed(mods):
        time.sleep(0.005)
        _post_key(hwnd, _MOD_VK[m], False, is_sys=is_sys)
...
class AutoClicker:
    ...
    def _send_extra_keys(self, hwnd):
        ...
        for spec in self.settings.extra_keys:
            ...
            time.sleep(0.01)  # pequeno intervalo entre teclas
